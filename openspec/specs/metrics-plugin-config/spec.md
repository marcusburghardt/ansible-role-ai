...
The role SHALL provide an `ai_opencode_metrics_version` variable (default:
`"0.2.0"`) that specifies the pinned version of the `@mburghardt/opencode-metrics`
npm package. This variable SHALL be referenced by the default
`ai_opencode_plugins` list to construct the versioned package specifier.
