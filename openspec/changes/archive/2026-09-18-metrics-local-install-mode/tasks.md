...
- [x] 1.1 Add `ai_opencode_metrics_local_path: ""` to `defaults/main.yml` with a
...
- [x] 1.2 Update the `ai_opencode_plugins` default in `defaults/main.yml` to the
...
- [x] 1.3 Add the computed variable `_ai_metrics_local_path_expanded` to
...
- [x] 2.1 Create `templates/opencode_metrics_plugin_loader.ts.j2` containing the
...
- [x] 3.1 Add a validation task to `tasks/configure_opencode.yml` (before any loader
...
- [x] 3.2 Add a debug task to `tasks/configure_opencode.yml` (after validation,
...
- [x] 3.3 Add a task to `tasks/configure_opencode.yml` that removes
...
- [x] 3.4 Add a task to `tasks/configure_opencode.yml` that writes
...
- [x] 4.1 Add a "Local development of opencode-metrics" section to `README.md`
...
- [x] 4.2 Add a "Switching back to npm mode" note in the same README section
...
- [x] 5.1 Run `ansible-lint` against the role and confirm zero new lint issues
  introduced by this change.

- [x] 5.2 Run `openspec validate --change metrics-local-install-mode` and confirm the
